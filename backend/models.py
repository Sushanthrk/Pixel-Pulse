...
class GeoRecommendation(BaseModel):
    """Structured action plan generated by an LLM for a single GEO query."""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=_id)
    client_id: str
    query_id: str
    query: str
    summary: str = ""              # diagnosis paragraph
    estimated_months: int = 6      # projected time to first mention
    confidence: str = "Medium"     # Low / Medium / High
    actions: list = []             # [{title, rationale, effort, timeframe}]
    timeline: list = []            # [{month: int, milestone: str}]
    raw_payload: str = ""          # full LLM response if parsing fails
    created_at: datetime = Field(default_factory=_now)
